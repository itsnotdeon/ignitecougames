...
function read(){try{const raw=JSON.parse(localStorage.getItem(KEY)||"{}");const vibe=Array.isArray(raw.vibes)&&raw.vibes.length?raw.vibes[0]:DEFAULT.vibes[0];const legacy=raw.steps??(raw.duration==="short"?4:raw.duration==="long"?6:5);const steps=Math.max(3,Math.min(12,Number(legacy)||DEFAULT.steps));return{vibes:[VIBES.includes(vibe)?vibe:DEFAULT.vibes[0]],steps}}catch{return{...DEFAULT}}}
export function getPreferences(){const p=read();return{vibes:[p.vibes?.[0]||DEFAULT.vibes[0]],steps:Math.max(3,Math.min(12,Number(p.steps)||DEFAULT.steps))}}
