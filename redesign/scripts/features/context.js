...
export function buildContext({date=new Date(),mood=getMood(),level=1,preferences={},memories=[]}={}){const time=getTimeContext(date);const moodMeta=getMoodMeta(mood);const recent=Array.isArray(memories)?memories.slice(0,3):[];return{time,mood:moodMeta?{id:moodMeta.id,label:moodMeta.label,energy:moodMeta.energy,intention:moodMeta.intention}:null,level:Number(level)||1,preferences:{vibes:Array.isArray(preferences.vibes)?preferences.vibes:[],duration:preferences.duration||"medium",intensity:preferences.intensity||"balanced"},recentJourneys:recent.map(item=>item.journey).filter(Boolean)}}