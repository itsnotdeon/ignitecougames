...
export function nextRoleplay(s){const roles=s.mode==="dark"?darkRoles():normalRoles();const role=s.role&&((s.usedItems.get(s.role.id)||new Set()).size< s.role.items.length)?s.role:pick(roles,s.usedRoles);let used=s.usedItems.get(role.id)||new Set();if(used.size>=role.items.length){used=new Set();s.usedItems.set(role.id,used)}const idx=Math.floor(Math.random()*role.items.length);used.add(idx);s.usedItems.set(role.id,used);s.role=role;s.item=role.items[idx];return s}
