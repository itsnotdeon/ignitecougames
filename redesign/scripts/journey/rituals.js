...
export function createGuessColorState(){return{turn:0,roundsPlayed:0,targetRounds:5,currentSuit:pickSuit(),revealed:false};}
...
export function nextGuessRound(state){if(state.roundsPlayed>=state.targetRounds)return;state.turn=state.turn===0?1:0;state.currentSuit=pickSuit();state.revealed=false}
