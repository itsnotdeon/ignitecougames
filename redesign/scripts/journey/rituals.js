...
export function createRpsState(){return{stage:"p1",p1Move:null,p2Move:null,roundsPlayed:0,targetRounds:5};}
...
export function createGuessColorState(){return{turn:0,roundsPlayed:0,targetRounds:5,currentSuit:pickSuit(),revealed:false,lastGuess:null};}
...
export function guessColor(state,color){if(state.revealed)return null;const correct=state.currentSuit.color===color;state.lastGuess=color;state.revealed=true;state.roundsPlayed++;return correct}
export function nextGuessRound(state){if(state.roundsPlayed>=state.targetRounds)return;state.turn=state.turn===0?1:0;state.currentSuit=pickSuit();state.revealed=false;state.lastGuess=null}
